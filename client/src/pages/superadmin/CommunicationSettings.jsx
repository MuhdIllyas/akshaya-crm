...
import { FiPlus, FiMessageSquare, FiSettings, FiCopy, FiCheck } from "react-icons/fi";

const CommunicationSettings = () => {
  const [activeTab, setActiveTab] = useState("accounts"); // 'accounts' or 'templates'
  ...
  // Dummy event keys that your CRM code relies on
  const systemEvents = [
  ...
  const fetchAccounts = async () => {
    setLoading(true);
    try {
      // Replace with your actual GET route once built
      ...
      // Fallback dummy data for UI testing before backend is ready
      setAccounts([
  ...
  const copyWebhook = (phone) => {
    // Generates the webhook URL the Superadmin needs to put into Libromi
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Webhook URL</th>
                  </tr>
                </thead>
                    ...
                    <tr key={acc.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <button 
                          ...
                          className="text-gray-500 hover:text-[#1e3a5f] flex items-center justify-end gap-1 w-full"
                          title="Copy Webhook URL for Libromi"
                        >
                          {copiedToken ? <FiCheck className="text-green-500" /> : <FiCopy />}
                          <span className="text-xs">Copy Webhook</span>
                        </button>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="bg-[#1e3a5f] text-white p-4">
              <h2 className="text-lg font-semibold">Add New WhatsApp Account</h2>
            </div>
            <form onSubmit={handleCreateAccount} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Libromi Access Token</label>
                  <input
                    type="password"
                    required
