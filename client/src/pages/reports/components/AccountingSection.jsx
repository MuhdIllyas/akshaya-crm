...
const ExpenseManagement = ({ expenses, onApprove, onReject, allowAdd = true, onAddClick }) => {
  ...
  const handleSubmitExpense = () => {
    ...
    const expense = {
      ...
      submittedBy: 'Admin',
      status: newExpense.requiresApproval ? 'pending' : 'approved'
    };
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-bold text-gray-900 text-sm flex items-center">
                <FiPlusCircle className="h-4 w-4 mr-2 text-indigo-600" />
                Add New Expense
              </h4>
              <button
            ...
            <div className="grid gap-3 grid-cols-2">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center">
                  <FiFileText className="h-3 w-3 mr-1 text-gray-500" />
                  Description *
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center">
                  <FiDollarSign className="h-3 w-3 mr-1 text-gray-500" />
                  Amount (₹) *
                </label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center">
                  <FiTag className="h-3 w-3 mr-1 text-gray-500" />
                  Category
                </label>
                <select
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center">
                  <FiCreditCard className="h-3 w-3 mr-1 text-gray-500" />
                  Payment Method
                </label>
                <select
              ...
              <div className="col-span-2">
                <label className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={newExpense.requiresApproval}
                    onChange={(e) => setNewExpense(prev => ({ ...prev, requiresApproval: e.target.checked }))}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="text-xs text-gray-700 flex items-center">
                    <FiShield className="h-3 w-3 mr-1" />
                    Requires Approval (for staff-submitted expenses)
                  </span>
                </label>
              </div>
              <div className="col-span-2">
      ...
      <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white rounded-lg border border-emerald-200 p-3">
            ...
            <div className="space-y-1">
              {approvedExpenses.slice(0, 3).map(expense => (
                <div
                  key={expense.id}
                  className="p-2 rounded bg-emerald-50 border border-emerald-100"
                >
                  <div className="flex justify-between items-center">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 truncate">
                        {expense.date} • {expense.staff_name || 'Admin'}
                      </p>
                    </div>
                    <div className="font-semibold text-emerald-700 text-xs whitespace-nowrap ml-2">
        ...
        {rejectedExpenses.length > 0 && (
          <div className="bg-white rounded-lg border border-rose-200 p-3">
            <h4 className="font-semibold text-gray-900 text-sm mb-2 flex items-center">
              <FiXCircle className="h-3 w-3 mr-1 text-rose-600" />
              Rejected ({rejectedExpenses.length})
            </h4>
            <div className="space-y-1">
              {rejectedExpenses.slice(0, 3).map(expense => (
                <div
                  key={expense.id}
                  className="p-2 rounded bg-rose-50 border border-rose-100"
                >
                  <div className="flex justify-between items-center">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 text-xs truncate">
                    ...
                    <div className="font-semibold text-rose-700 text-xs whitespace-nowrap ml-2">
                      ₹{Number(expense.amount).toLocaleString('en-IN', {
                        minimumFractionDigits: 2
                      })}
                    </div>
...
// Main Accounting Section Component
const AccountingSection = ({ 
  ...
  // Helper function to build query string with centreId for superadmin
