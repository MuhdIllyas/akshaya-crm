...
import { 
  ...
  FiChevronRight, FiChevronsDown, FiLayers, FiActivity, FiZap,
      ...
      return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-xl border border-gray-200 p-8 max-w-md text-center">
            ...
            <p className="text-gray-600 mb-4">There was an error loading the service tracking.</p>
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-[1600px] mx-auto px-6 py-5">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-11 h-11 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl flex items-center justify-center shadow-md flex-shrink-0">
                  <FiTarget className="h-5 w-5 text-white" />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h1 className="text-xl font-bold text-gray-900 truncate">Service Management</h1>
                    <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 font-semibold border border-indigo-100">
                      Live
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">Track, edit and manage every service application in one place</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button onClick={() => window.location.reload()}
                  className="flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-br from-indigo-600 to-purple-600 text-white text-sm font-medium rounded-lg hover:from-indigo-700 hover:to-purple-700 shadow-sm transition-all">
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <SlimKpiCard title="Total Services" value={globalStats.total || 0} trend={12} icon={FiBarChart2} accent="blue" />
            <SlimKpiCard title="In Progress" value={globalStats.in_progress || 0} trend={8} icon={FiTrendingUp} accent="amber" />
            <SlimKpiCard title="Completed" value={globalStats.completed || 0} trend={15} icon={FiCheckCircle} accent="emerald" />
            <SlimKpiCard title="SLA Compliance" value={`${Math.round(globalStats.sla_compliance || 100)}%`} trend={2} icon={FiAward} accent="purple" />
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="p-3.5 flex flex-col lg:flex-row lg:items-center gap-3">
              ...
              <div className="relative flex-1 min-w-0">
                ...
                <input
                  type="text"
                  placeholder="Search by customer name, phone, application no, Aadhaar, service…"
                  className="w-full pl-10 pr-10 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-all"
                  value={searchTerm}
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
                <StatusChip active={statusFilter === 'all'} onClick={() => setStatusFilter('all')} label="All" count={globalStats.total} />
                <StatusChip active={statusFilter === 'Pending'} onClick={() => setStatusFilter('Pending')} label="Pending" dot="bg-amber-500" />
                <StatusChip active={statusFilter === 'In Progress'} onClick={() => setStatusFilter('In Progress')} label="Active" dot="bg-blue-500" />
                <StatusChip active={statusFilter === 'Completed'} onClick={() => setStatusFilter('Completed')} label="Done" dot="bg-emerald-500" />
                <StatusChip active={statusFilter === 'Delayed'} onClick={() => setStatusFilter('Delayed')} label="Delayed" dot="bg-rose-500" />
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  ...
                  Filters
                  {[staffFilter, expiryFilter, serviceFilter, subcategoryFilter].filter(f => f !== 'all').length + (dateFilter ? 1 : 0) + (aadhaarSearch ? 1 : 0) > 0 && (
                    <span className={`ml-0.5 px-1.5 py-0.5 text-[10px] font-bold rounded-full ${
                      ...
                      {[staffFilter, expiryFilter, serviceFilter, subcategoryFilter].filter(f => f !== 'all').length + (dateFilter ? 1 : 0) + (aadhaarSearch ? 1 : 0)}
                    </span>
                  )}
                </button>
              </div>
                ...
                <motion.div
                  ...
                  <div className="p-3.5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <FilterField label="Subcategory">
                      <select
                        className={`filter-input ${serviceFilter === 'all' ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <FilterField label="Aadhaar Search">
                      <div className="relative">
                        <FiCreditCard className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 h-3.5 w-3.5" />
                        <input type="text" placeholder="12-digit Aadhaar" className="filter-input pl-9" value={aadhaarSearch} onChange={(e) => setAadhaarSearch(e.target.value)} maxLength="12" />
                      </div>
                    </FilterField>
            ...
            {/* Result summary */}
            <div className="px-3.5 py-2 bg-gray-50/60 border-t border-gray-100 rounded-b-xl flex items-center justify-between text-xs text-gray-500">
              <span>
                Showing <span className="font-bold text-gray-800">{services.length}</span> of <span className="font-bold text-gray-800">{totalRecords}</span> services
              </span>
              <span className="hidden sm:inline">
                Updated just now · Page {currentPage} of {totalPages}
              </span>
            </div>
          </div>
          ...
          {viewMode === 'grid' ? (
                  ...
                  <tbody>
                    {Object.entries(servicesByDate).map(([date, dateServices]) => (
                      <React.Fragment key={date}>
                        {/* Date group header */}
                        <tr className="bg-gradient-to-r from-gray-50 to-white border-y border-gray-100">
                        ...
                        {dateServices.map(service => (
                          <React.Fragment key={service.id}>
                            <tr className={`group border-b border-gray-100 transition-colors ${
                              selectedService?.id === service.id ? 'bg-indigo-50/40' : 'hover:bg-gray-50/60'
                            }`}>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-3">
                                  <div className="relative flex-shrink-0">
                                    ...
                                    <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${statusConfig[service.status]?.dot || 'bg-gray-400'}`}></span>
                                  </div>
                                  <div className="min-w-0">
                                    <div className="text-sm font-semibold text-gray-900 truncate max-w-[180px]" title={service.customerName}>
                                    ...
                                    <div className="flex items-center gap-1.5 text-[11px] text-gray-500 mt-0.5">
                                      ...
                                      {service.workSource === 'online' ? (
                                        <span className="ml-1 text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 bg-green-100 text-green-700 rounded">Online</span>
                                      ) : (
                                        <span className="ml-1 text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded">Walk-in</span>
                                      )}
                                    </div>
                                  </div>
                                </div>
                              ...
                              {/* Service cell */}
                              <td className="px-4 py-3">
                                <div className="text-sm font-medium text-gray-900 truncate max-w-[180px]" title={service.serviceType}>
                                  {service.serviceType}
                                </div>
                                <div className="flex items-center gap-1.5 mt-1">
                                  <span className="text-[11px] text-gray-500 truncate max-w-[110px]" title={service.subcategoryName}>
                                    {service.subcategoryName || '-'}
                                  </span>
                                  <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide ${priorityConfig[service.priority]?.bg} ${priorityConfig[service.priority]?.color} border ${priorityConfig[service.priority]?.border}`}>
                              ...
                              <td className="px-4 py-3">
                                <input 
                                  ...
                                  className="w-full text-xs font-mono font-medium text-gray-800 border border-transparent hover:border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 rounded-md px-2 py-1.5 bg-gray-50/50 focus:bg-white transition-all"
                                  placeholder="Set app no..."
                                />
                                <div className="text-[10px] text-gray-400 font-mono px-2 mt-0.5">
                                  {service.trackingId}
                                </div>
                              </td>
                              ...
                              <td className="px-4 py-3 space-y-1.5">
                                <select
                                  ...
                                  className={`w-full text-[11px] font-bold rounded-md px-2 py-1.5 border outline-none shadow-sm cursor-pointer transition-all ${statusConfig[service.status]?.bg || 'bg-gray-100'} ${statusConfig[service.status]?.color || 'text-gray-800'} ${statusConfig[service.status]?.border || 'border-gray-200'}`}>
                                ...
                                <select
                                  ...
                                  className="w-full text-[11px] font-medium text-gray-600 bg-white border border-gray-300 rounded-md px-2 py-1 shadow-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 cursor-pointer transition-all">
                              ...
                              <td className="px-4 py-3 space-y-1.5">
                                <select
                                  ...
                                  className="w-full text-[11px] font-medium text-gray-700 bg-white border border-gray-300 rounded-md px-2 py-1 shadow-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 cursor-pointer transition-all">
                              ...
                              <td className="px-4 py-3 align-top">
                                <div className="flex items-center justify-end gap-1.5 mt-0.5">
                                  <button
                                    ...
                                    className="p-1.5 rounded-lg transition-all border shadow-sm bg-white text-gray-500 border-gray-200 hover:text-green-600 hover:bg-green-50 hover:border-green-200">
                                  ...
                                  <button 
                                    ...
                                    title={selectedService?.id === service.id ? "Collapse" : "Expand Details"}
                                    ...
                                    <FiChevronDown className={`h-3.5 w-3.5 transform transition-transform duration-300 ${selectedService?.id === service.id ? 'rotate-180' : ''}`} />
                                  </button>
                                </div>
                              </td>
                            </tr>
                            ...
                            {selectedService?.id === service.id && (
                              <tr>
                                <td colSpan="6" className="p-0 bg-gray-50/60">
                                  <motion.div
                                    initial={{ opacity: 0, height: 0 }}
                                    animate={{ opacity: 1, height: 'auto' }}
                                    className="overflow-hidden border-b-2 border-indigo-200">
                                    <div className="p-5">
                                      <DetailPane
                                        ...
                                        onTrackingFormSubmit={handleTrackingFormSubmit}
                                        onUpload={handleUploadDocument}
                                        onUpdateRemark={handleUpdateDocumentRemark}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
                  <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}
                    className="px-3.5 py-2 text-xs font-semibold text-gray-700 bg-white rounded-lg border border-gray-200 hover:bg-gray-50 hover:text-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm">
                    ← Previous
                  </button>
                  <div className="text-xs text-gray-600 font-medium bg-white px-3.5 py-1.5 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
              <div className="xl:col-span-1 bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex flex-col max-h-[calc(100vh-300px)]">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-gray-900 text-sm">Services <span className="text-gray-400 font-normal">({totalRecords})</span></h3>
                </div>
                <div className="space-y-2 overflow-y-auto scrollbar-hide flex-1">
                  {services.map(service => (
                    <CompactServiceCard key={service.id} service={service}
                      ...
                      statusConfig={statusConfig} priorityConfig={priorityConfig} />
                  ))}
                  {services.length === 0 && (
              ...
              <div className="xl:col-span-3">
                {selectedService ? (
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">No Service Selected</h3>
                    <p className="text-sm text-gray-500 max-w-sm mx-auto">
                      Select a service from the list to view details, edit inline, manage documents and track history
                    </p>
                  </div>
                )}
              </div>
...
const SlimKpiCard = ({ title, value, trend, icon: Icon, accent }) => {
  const accents = {
  ...
  return (
    <motion.div whileHover={{ y: -2 }}
      className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-all duration-300 group relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${accents[accent]} opacity-[0.06] rounded-full -mr-8 -mt-8`}></div>
      <div className="flex items-start justify-between gap-3 relative">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-wider font-semibold text-gray-500 mb-1">{title}</p>
          <p className="text-2xl font-bold text-gray-900 leading-none">{value}</p>
          <div className="flex items-center gap-1 mt-2">
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${trend > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
              {trend > 0 ? '↑' : '↓'} {Math.abs(trend)}%
            </span>
            <span className="text-[10px] text-gray-400">vs last period</span>
          </div>
        </div>
        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${accents[accent]} flex items-center justify-center shadow-sm flex-shrink-0 group-hover:scale-110 transition-transform`}>
...
const StatusChip = ({ active, onClick, label, dot, count }) => (
  <button onClick={onClick}
    ...
    <span>{label}</span>
    {count !== undefined && (
      <span className={`ml-0.5 px-1.5 py-0.5 text-[9px] font-bold rounded-full ${active ? 'bg-white/25' : 'bg-gray-100 text-gray-600'}`}>
        {count}
      </span>
    )}
  </button>
...
const CompactServiceCard = ({ service, isSelected, onClick, statusConfig, priorityConfig }) => {
    ...
    <motion.div whileHover={{ x: 2 }}
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold text-gray-900 truncate">{service.customerName}</p>
            <p className="text-[10px] text-gray-500 truncate">{service.phone}</p>
          </div>
        </div>
      </div>
      <p className="text-[11px] font-medium text-gray-800 truncate mb-1.5">{service.serviceType}</p>
      <div className="flex items-center justify-between gap-2">
        <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${config.bg} ${config.color} border ${config.border}`}>
          {service.status}
        </span>
        <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-0.5 ${priority.bg} ${priority.color} border ${priority.border}`}>
          <FiFlag className="h-2 w-2" />
          {priority.label}
        </span>
      </div>
...
/* ============================================================
   ...
   ============================================================ */
const DetailPane = ({
  service, activeTab, setActiveTab, trackingFormData, onFormChange,
  staffList, stepOptions, priorityOptions, documents, documentsLoading,
  uploadingDocument, activityHistory, activityLoading,
  onNotify, onUpdateStatus, onTrackingFormSubmit,
  onUpload, onUpdateRemark, onToggleVisibility, onDeleteDoc,
  statusConfig, priorityConfig, paymentStatusConfig, navigate
}) => {
  const tabs = [
  ...
  return (
    <div className="space-y-5">
      {/* Hero card */}
      <div className="bg-gradient-to-br from-indigo-600 via-indigo-600 to-purple-700 rounded-xl p-5 text-white shadow-lg relative overflow-hidden">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            <div className="w-14 h-14 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center flex-shrink-0 border border-white/20">
              <span className="text-white text-xl font-bold">{(service.customerName || 'U').charAt(0).toUpperCase()}</span>
            </div>
            <div className="min-w-0">
              <h2 className="text-xl font-bold truncate">{service.customerName || 'Unknown'}</h2>
              <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-white/90">
                <span className="flex items-center gap-1"><FiPhone className="h-3 w-3" />{service.phone || 'N/A'}</span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${
              service.workSource === 'online' ? 'bg-green-400/20 text-green-100 border border-green-300/30' : 'bg-blue-400/20 text-blue-100 border border-blue-300/30'
            }`}>
              {service.workSource === 'online' ? 'Online' : 'Walk-in'}
            </span>
            <button onClick={() => onNotify(service)}
              className="flex items-center gap-1.5 px-3 py-2 bg-white/15 hover:bg-white/25 backdrop-blur text-xs font-semibold rounded-lg border border-white/20 transition-colors">
              <FiMessageSquare className="h-3.5 w-3.5" />
              Notify
            </button>
            <button onClick={() => navigate(`/dashboard/staff/service-workspace/${service.id}`)}
              className="flex items-center gap-1.5 px-3 py-2 bg-white text-indigo-700 hover:bg-indigo-50 text-xs font-semibold rounded-lg shadow-sm transition-colors">
              <FiGrid className="h-3.5 w-3.5" />
              Workspace
            </button>
          </div>
        ...
        <div className="relative grid grid-cols-2 lg:grid-cols-4 gap-2 mt-5">
          <div className="bg-white/10 backdrop-blur rounded-lg p-2.5 border border-white/15">
            <p className="text-[9px] uppercase tracking-wider text-white/70 font-semibold">Application No</p>
            <p className="text-xs font-bold mt-0.5 truncate">{service.applicationNumber || 'N/A'}</p>
          </div>
          <div className="bg-white/10 backdrop-blur rounded-lg p-2.5 border border-white/15">
            <p className="text-[9px] uppercase tracking-wider text-white/70 font-semibold">Current Step</p>
            <p className="text-xs font-bold mt-0.5 truncate">{service.currentStep || 'N/A'}</p>
          </div>
          <div className="bg-white/10 backdrop-blur rounded-lg p-2.5 border border-white/15">
            <p className="text-[9px] uppercase tracking-wider text-white/70 font-semibold">Est. Delivery</p>
            <p className="text-xs font-bold mt-0.5 truncate">{service.estimatedDelivery || 'Not set'}</p>
          </div>
          <div className="bg-white/10 backdrop-blur rounded-lg p-2.5 border border-white/15">
            <p className="text-[9px] uppercase tracking-wider text-white/70 font-semibold">Assigned To</p>
            <p className="text-xs font-bold mt-0.5 truncate">{service.assignedTo || 'Unassigned'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-5">
          <AnimatePresence mode="wait">
            <motion.div key={activeTab}
              ...
              {activeTab === 'overview' && (
                <OverviewTab service={service} onUpdateStatus={onUpdateStatus} priorityConfig={priorityConfig} paymentStatusConfig={paymentStatusConfig} statusConfig={statusConfig} />
              )}
              {activeTab === 'tracking' && (
                <TrackingTab service={service} formData={trackingFormData} onFormChange={onFormChange}
                  staffList={staffList} stepOptions={stepOptions} priorityOptions={priorityOptions}
                  onSave={onTrackingFormSubmit} onCancel={() => setActiveTab('overview')} />
              )}
              {activeTab === 'documents' && (
                <EnhancedDocumentsView documents={documents} documentsLoading={documentsLoading}
                  uploadingDocument={uploadingDocument}
                  onUpload={(file, label, visible, remark) => onUpload(service.id, file, label, visible, remark)}
                  onUpdateRemark={(docId, remark) => onUpdateRemark(service.id, docId, remark)}
                  onToggleVisibility={(docId, visible) => onToggleVisibility(service.id, docId, visible)}
                  onDelete={(docId) => onDeleteDoc(service.id, docId)} />
              )}
              {activeTab === 'history' && (
                <HistoryTab activityHistory={activityHistory} activityLoading={activityLoading} />
              )}
...
/* ============================================================
   OVERVIEW TAB
   ============================================================ */
const OverviewTab = ({ service, onUpdateStatus, priorityConfig, paymentStatusConfig, statusConfig }) => {
        ...
        <div className="lg:col-span-2 space-y-5">
          <div>
            ...
            <div className="grid grid-cols-4 gap-2 mt-2">
              {['Submitted', 'Initial Review', 'Doc Verification', 'Final Approval'].map((label, i) => (
                <div key={i} className={`text-[10px] font-medium text-center py-1 rounded transition-colors ${
                  service.progress >= (i + 1) * 25 ? 'text-indigo-600' : 'text-gray-400'
                }`}>
                  {label}
                </div>
              ))}
          ...
          <div>
            <h4 className="font-semibold text-gray-900 text-sm mb-3">Update Status</h4>
            <div className="flex flex-wrap gap-2">
              {Object.keys(statusConfig).map(statusKey => {
                const isCurrent = service.status === statusKey;
                return (
                  <button key={statusKey}
                    className={`px-3 py-2 rounded-lg flex items-center gap-1.5 text-xs font-semibold transition-all ${
                      isCurrent
                        ? 'bg-gray-600 text-white cursor-not-allowed opacity-75'
                        : `${statusConfig[statusKey].button} hover:shadow-md hover:-translate-y-0.5`
                    }`}
                    onClick={() => onUpdateStatus(service.id, statusKey)}
                    disabled={isCurrent}>
                    {isCurrent && <FiCheckCircle className="h-3.5 w-3.5" />}
                    {statusKey}
                  </button>
                );
              })}
              ...
              {displaySteps.map((step) => {
                ...
                return (
                  <div key={step.id} className="relative flex items-start gap-3">
                    <div className={`absolute -left-[13px] top-1 w-3 h-3 rounded-full border-2 border-white shadow-sm ${
                      isCompleted ? 'bg-emerald-500' : isCurrent ? 'bg-indigo-500 animate-pulse' : 'bg-gray-300'
                    }`}></div>
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center py-1">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 ${priorityConfig[service.priority]?.bg} ${priorityConfig[service.priority]?.border} border`}>
                <FiFlag className={`h-2.5 w-2.5 ${priorityConfig[service.priority]?.color}`} />
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <span key={i} className={i < service.serviceRating ? 'text-yellow-400 text-base' : 'text-gray-300 text-base'}>★</span>
                ))}
            ...
            {service.staffRating && (
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-gray-600 font-medium">Staff:</span>
                <div className="flex items-center gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <span key={i} className={i < service.staffRating ? 'text-yellow-400 text-base' : 'text-gray-300 text-base'}>★</span>
                  ))}
            ...
            {service.reviewText && (
              <div className="mt-2">
                ...
                <p className="text-sm text-gray-700 bg-white p-3 rounded-lg border border-amber-200 italic">
                  "{service.reviewText}"
                </p>
              </div>
            )}
...
/* ============================================================
   TRACKING TAB (same as original)
   ============================================================ */
const TrackingTab = ({ service, formData, onFormChange, staffList, stepOptions, priorityOptions, onSave, onCancel }) => (
  <div className="space-y-5">
    <div className="flex items-center gap-2">
      <div className="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center">
    ...
    <form onSubmit={onSave} className="grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      <div className="md:col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-200">
        <button type="button" className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors" onClick={onCancel}>
          Cancel
        </button>
        <button type="submit" className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-br from-indigo-600 to-purple-600 text-white text-sm font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 shadow-sm transition-all">
...
/* ============================================================
   DOCUMENTS VIEW (enhanced, kept from previous)
   ============================================================ */
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 ml-11">Files marked visible and their remarks appear on the customer's tracking page</p>
        </div>
        <div className="flex items-center gap-2 ml-11 sm:ml-0">
          <div className="px-3 py-1.5 bg-gray-100 rounded-lg">
            <span className="text-xs font-semibold text-gray-700">{documents.length} {documents.length === 1 ? 'file' : 'files'}</span>
          </div>
          <div className="px-3 py-1.5 bg-emerald-50 rounded-lg border border-emerald-200">
            <span className="text-xs font-semibold text-emerald-700">{documents.filter(d => d.visible_to_customer).length} visible</span>
          </div>
        </div>
      </div>
            ...
            <div className="lg:col-span-3 flex flex-col">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={uploadingDocument || !file || !label.trim()}
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-gradient-to-br from-indigo-600 to-purple-600 text-white text-xs font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all">
                  {uploadingDocument ? (<><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>Uploading</>) : (<><FiUpload className="h-3.5 w-3.5" />Upload</>)}
                </button>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/70 border-b border-gray-200 text-[10px] text-gray-500 uppercase tracking-wider">
                  ...
                  <th className="px-5 py-3 font-semibold w-24 text-center">Size</th>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold text-gray-900 truncate" title={doc.label}>{doc.label}</p>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1">
                          <a href={doc.file_url || doc.url || '#'} target="_blank" rel="noopener noreferrer"
                            ...
                            title="View document" onClick={(e) => { if (!doc.file_url && !doc.url) e.preventDefault(); }}>
                            <FiExternalLink className="h-4 w-4" />
                          </a>
                          <button onClick={() => { if (window.confirm(`Delete "${doc.label}"? This cannot be undone.`)) onDelete(doc.id); }}
                            className="p-1.5 rounded-lg text-gray-400 hover:text-rose-600 hover:bg-rose-50 transition-colors" title="Delete document">
...
/* ============================================================
   HISTORY TAB
   ============================================================ */
const HistoryTab = ({ activityHistory, activityLoading }) => {
  if (activityLoading) {
    return (
      <div className="space-y-4">
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="text-center py-10 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <p className="text-xs text-gray-500">Activity will appear here as changes are made</p>
        </div>
      </div>
    );
  }
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-900 text-sm">Activity History</h3>
        <span className="text-xs text-gray-500 font-medium">{activityHistory.length} {activityHistory.length === 1 ? 'event' : 'events'}</span>
      </div>
      <div className="relative">
        ...
        <div className="space-y-3">
          {activityHistory.map((activity) => (
            <div key={activity.id} className="relative flex gap-3">
              ...
              <div className="flex-1 bg-white rounded-lg border border-gray-200 p-3 hover:shadow-sm transition-shadow">
