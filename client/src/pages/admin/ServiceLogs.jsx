...
// Chart Components - Service Oriented Only
...
const ServiceTypeDistributionChart = ({ services }) => {
  const serviceTypeData = useMemo(() => {
    ...
    // Get top 6 service types by count
...
const StaffWorkloadChart = ({ services }) => {
  const staffData = useMemo(() => {
    ...
    // Get staff with highest workload
...
const ServiceLogs = () => {
  ...
  const fetchServiceEntries = async () => {
    try {
      ...
      console.log('ServiceLogs: Fetched service entries:', JSON.stringify(entries, null, 2));
      return entries;
    } catch (error) {
      console.error('ServiceLogs: Error fetching service entries:', error);
      toast.error('Failed to fetch service entries: ' + (error.response?.data?.error || error.message));
      return [];
    }
  };

  // Fetch payment details for a specific service entry
  const fetchPaymentDetails = async (serviceEntryId, tokenId, serviceEntries) => {
    try {
      ...
      if (!serviceEntry) {
        console.warn(`ServiceLogs: No service entry found for serviceEntryId: ${serviceEntryId}, tokenId: ${tokenId}`);
      ...
      console.log('ServiceLogs: Payment details for serviceEntryId:', serviceEntryId, {
        totalReceived,
        totalCharge,
        paymentStatus,
        payments,
      ...
      console.error('ServiceLogs: Error processing payment details:', err);
  ...
  const handleStatusUpdate = async (serviceId, newStatus) => {
    try {
      console.log('ServiceLogs: Updating status for tracking ID:', serviceId, 'to:', newStatus);
  ...
  const handleServiceUpdate = async (serviceId, updates) => {
    try {
      console.log('ServiceLogs: Updating service with tracking ID:', serviceId);
      const backendUpdates = {
        ...updates,
        status: updates.status ? reverseStatusMap[updates.status] : undefined
      };
      
      await updateTrackingEntry(serviceId, backendUpdates);
      ...
      toast.error('Failed to update service: ' + (error.response?.data?.error || error.message));
    }
  };
  ...
  const handleNotifyCustomer = async (service) => {
    try {
      console.log('ServiceLogs: Sending notification for service:', {
        id: service.id,
        serviceEntryId: service.serviceEntryId,
        customerName: service.customerName,
        applicationNumber: service.applicationNumber,
      ...
      console.error('ServiceLogs: Error sending notification:', error);
  ...
  const ServiceListRow = ({ service, onClick }) => {
    const status = statusConfig[service.status] || statusConfig['Pending'];
    const priority = priorityConfig[service.priority] || priorityConfig['medium'];
      ...
      >
        <td className="py-4 px-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="font-medium text-gray-900">{service.customerName || 'Unknown'}</p>
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Track and monitor all service entries and tracking procedures</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <motion.button
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          {/* Charts Section - Service and Staff Focused Only */}
          {showCharts && (
            <motion.div
  ...
  const generateTimeline = (service) => {
    ...
    const formatDate = (date) => {
      if (!date || isNaN(new Date(date))) {
        console.warn(`ServiceLogs: Invalid date detected: ${date}`);
        return 'Not set';
      }
    ...
    console.log('ServiceLogs: Steps for service', service.id, JSON.stringify(steps, null, 2));
    ...
    steps.forEach((step, index) => {
      ...
      const estimatedDate = !step.completed && step.estimated_days 
        ? (() => {
            ...
            if (isNaN(baseDate)) {
              console.warn(`ServiceLogs: Invalid base date for step ${step.name}: ${baseDate}`);
              return 'Not set';
            }
