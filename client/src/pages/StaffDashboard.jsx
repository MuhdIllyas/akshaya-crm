...
const StaffDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ===== WELCOME BANNER (includes attendance widget) ===== */}
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm mt-0.5">Welcome back to your workspace.</p>
                <div className="flex items-center gap-4 text-sm text-white/90 mt-1">
                  <div className="flex items-center gap-2">
                    <span className="h-2 w-2 bg-green-400 rounded-full"></span>
                    <span>Online</span>
                  </div>
                  <button onClick={() => navigate('/dashboard/my-profile')} className="underline hover:text-white/80">View Profile</button>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button onClick={() => navigate('/dashboard/staff/token')} className="px-3 py-1.5 bg-white/20 rounded-lg text-sm font-medium hover:bg-white/30 transition">New Token</button>
                  <button onClick={() => setShowQuickService(true)} className="px-3 py-1.5 bg-white/20 rounded-lg text-sm font-medium hover:bg-white/30 transition">Quick Service</button>
                  <button onClick={() => navigate('/dashboard/staff/performance')} className="px-3 py-1.5 bg-white/20 rounded-lg text-sm font-medium hover:bg-white/30 transition">View Reports</button>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2">
            <div className="text-right">
              ...
              <p className="text-white/80 text-sm">{formatCurrentDate(currentTime)}</p>
            </div>
            {/* Attendance Widget (compact) */}
            <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 border border-white/20 shadow-md">
              <div className="flex items-center gap-2">
                {!todayAttendance || !todayAttendance.punch_in ? (
                  <FiAlertCircle className="h-5 w-5 text-rose-300" />
                ...
                <span className="text-xs font-medium">
                  {!todayAttendance || !todayAttendance.punch_in ? 'Not punched in' :
                   !todayAttendance.punch_out ? `Punched in at ${new Date(`1970-01-01T${todayAttendance.punch_in}`).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}` :
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm transition-all disabled:opacity-50 ${
                  !todayAttendance || !todayAttendance.punch_in ? 'bg-rose-500 hover:bg-rose-600' :
                  !todayAttendance.punch_out ? 'bg-gray-700 hover:bg-gray-800' :
                  'bg-amber-500 hover:bg-amber-600'
                }`}
              >
                {attendanceLoading ? '...' : (!todayAttendance || !todayAttendance.punch_in ? 'Punch In' : (!todayAttendance.punch_out ? 'Punch Out' : 'Punch In'))}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4 mb-6 items-center">
            ...
            {myWallet && (
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">{myWallet.name}</span>
                <span className="text-sm font-bold text-gray-900">{formatCurrency(myWallet.balance)}</span>
              </motion.div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {/* ===== LEFT: WORKSPACE ===== (spans 8 columns) */}
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col h-[850px]">
                <div className="flex p-3 border-b border-gray-200 bg-gray-50/80 gap-2 overflow-x-auto hide-scrollbar">
                  <button onClick={() => setWorkspaceTab('tokens')} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition-all ${workspaceTab === 'tokens' ? 'bg-white text-indigo-700 shadow-sm border border-gray-200' : 'text-gray-600 hover:bg-gray-200/50'}`}>
                    <FiUsers className="h-4 w-4" /> Walk-in Tokens
                  ...
                  <button onClick={() => setWorkspaceTab('queue')} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition-all ${workspaceTab === 'queue' ? 'bg-white text-blue-700 shadow-sm border border-gray-200' : 'text-gray-600 hover:bg-gray-200/50'}`}>
                    <FiGlobe className="h-4 w-4" /> Online Queue
                  ...
                  <button onClick={() => setWorkspaceTab('processing')} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition-all ${workspaceTab === 'processing' ? 'bg-white text-emerald-700 shadow-sm border border-gray-200' : 'text-gray-600 hover:bg-gray-200/50'}`}>
                    <FiPlayCircle className="h-4 w-4" /> My Online Work
                    ...
                    <div className="space-y-4">
                      <div className="flex gap-2 border-b border-gray-200 pb-3">
                        {['active', 'completed', 'campaign'].map(tab => (
                          <button key={tab} onClick={() => setActiveView(tab)} className={`px-4 py-1.5 rounded-full text-xs font-bold capitalize transition-colors ${activeView === tab ? 'bg-gray-800 text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-100'}`}>
                            {tab}
                          </button>
                        ))}
                      </div>
                      {Object.keys(groupedTokens).length === 0 ? (
                        <div className="text-center py-16 text-gray-400"><FiPlayCircle className="mx-auto h-12 w-12 mb-3 opacity-30"/><p className="font-medium">No tokens found</p></div>
                      ) : (
                                  ...
                                  return (
                                    <div key={token.tokenId} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl hover:shadow-md hover:border-indigo-300 transition-all group">
                                      <div className="flex items-center gap-3 min-w-0 flex-1">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 ${activeView === 'completed' ? 'bg-green-50 text-green-700 border border-green-100' : activeView === 'campaign' ? 'bg-purple-50 text-purple-700 border border-purple-100' : 'bg-indigo-50 text-indigo-700 border border-indigo-100'}`}>
                                      ...
                                      <div className="flex items-center gap-2 pl-3 shrink-0">
                                        {(activeView === 'active' || activeView === 'campaign') && (token.status === 'pending' || token.status === 'in-progress') && (isAssignedToMe || isUnassigned) && (
                                          <>
                                            <button onClick={() => handleStartService(token.tokenId, token.staffId, token.status)} className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-1.5 text-xs font-semibold shadow-sm transition-colors">
                                              <FiPlayCircle className="h-3.5 w-3.5" /> {token.status === 'pending' ? 'Start' : 'Resume'}
                                            </button>
                                            <button onClick={() => openCancelModal(token)} className="p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-colors" title="Cancel Token">
                  ...
                  {workspaceTab === 'queue' && (
                    <div className="space-y-3">
                      {onlineBookings.length === 0 ? (
                        <div className="text-center py-16 text-gray-400"><FiGlobe className="mx-auto h-12 w-12 mb-3 opacity-30"/><p className="font-medium">No pending online bookings</p></div>
                      ) : (
                        onlineBookings.map(booking => (
                          <div key={booking.id} className="flex items-center justify-between p-4 bg-white border border-blue-100 rounded-xl hover:shadow-md transition-all group">
                            ...
                            <button onClick={() => handleTakeWork(booking.id)} className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 shadow-sm shrink-0 transition-colors">Take Work</button>
                          </div>
                        ))
                      )}
                    </div>
                  ...
                  {workspaceTab === 'processing' && (
                    <div className="space-y-3">
                      {processingBookings.length === 0 ? (
                        <div className="text-center py-16 text-gray-400"><FiCheckSquare className="mx-auto h-12 w-12 mb-3 opacity-30"/><p className="font-medium">No active online work</p></div>
                      ) : (
            ...
            {/* ===== RIGHT SIDEBAR: BENTO CARDS ===== (spans 4 columns, further split into 2 columns) */}
            <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6">

              {/* Row 1: Performance Score & Metrics (stacked) */}
              <div className="space-y-6">
                  ...
                  <div className="text-center">
                    <div className="relative inline-block">
                      <svg className="w-32 h-32">
                        ...
                        <circle className="transition-all duration-1000" strokeWidth="12" strokeDasharray={339.292} strokeDashoffset={339.292 * (1 - (performanceLoading ? 0 : performance.incentiveScore / 100))} strokeLinecap="round" stroke={`url(#gradient)`} fill="transparent" r="54" cx="64" cy="64" />
                        <defs>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  {performanceLoading ? (
                    <div className="space-y-3"><div className="h-8 bg-gray-200 animate-pulse rounded"></div><div className="h-8 bg-gray-200 animate-pulse rounded"></div><div className="h-8 bg-gray-200 animate-pulse rounded"></div></div>
                  ) : (
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between mb-1 text-sm"><span className="text-gray-600">Completion Rate</span><span className="font-medium text-gray-900">{performance.completionRate}%</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-green-600 h-2 rounded-full" style={{ width: `${performance.completionRate}%` }} /></div>
                      </div>
                      <div>
                        <div className="flex justify-between mb-1 text-sm"><span className="text-gray-600">Avg Transaction Value</span><span className="font-medium text-gray-900">{formatCurrency(performance.avgTransactionValue)}</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${Math.min((performance.avgTransactionValue / 1000) * 100, 100)}%` }} /></div>
                      </div>
                      <div>
                        <div className="flex justify-between mb-1 text-sm"><span className="text-gray-600">Customer Satisfaction</span><span className="font-medium text-gray-900">{performance.customerSatisfaction}</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-purple-600 h-2 rounded-full" style={{ width: `${(performance.avgRating / 5) * 100}%` }} /></div>
                      </div>
                      <div className="pt-2 border-t border-gray-100">
                        <div className="flex justify-between text-sm"><span className="text-gray-600">Collection Rate</span><span className="font-medium text-gray-900">{performance.collectionRate}%</span></div>
                        <div className="flex justify-between text-sm mt-2"><span className="text-gray-600">Total Revenue</span><span className="font-medium text-gray-900">{formatCurrency(performance.totalCollected)}</span></div>
                        <div className="flex justify-between text-sm mt-2"><span className="text-gray-600">Total Services</span><span className="font-medium text-gray-900">{performance.totalServices}</span></div>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="bg-indigo-100 text-indigo-800 text-xs font-bold px-2 py-0.5 rounded-full">{myTasks.length}</span>
                  </div>
                  {myTasks.length === 0 ? (
                    <div className="text-center py-4 bg-gray-50 rounded-lg border border-dashed border-gray-200"><p className="text-sm text-gray-500">No pending tasks! 🎉</p></div>
                  ) : (
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                      {myTasks.map(task => (
                        <div key={task.id} className="flex gap-3 items-start p-3 border border-gray-100 rounded-lg bg-gray-50 hover:bg-white transition shadow-sm">
                          <button onClick={() => handleCompleteTask(task.id)} className="mt-0.5 text-gray-400 hover:text-emerald-500 transition-colors" title="Mark as complete">
                            <FiCheckCircle className="h-5 w-5" />
                          </button>
                          <div className="flex-1">
                            <p className="text-sm font-medium text-gray-800">{task.title}</p>
                            {task.due_date && <p className="text-xs text-rose-500 mt-1 flex items-center gap-1 font-medium"><FiClock className="h-3 w-3" /> Due {getEventDayLabel(task.due_date)}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                >
                  ...
                  <div className="flex gap-2 mb-4 overflow-x-auto hide-scrollbar pb-1">
                    {['All', 'Tasks', 'Deliveries', 'Expiries'].map(tab => {
                      ...
                      return (
                        <button key={tab} onClick={() => setActiveEventTab(tab)} className={`text-xs px-3 py-1.5 rounded-full font-medium whitespace-nowrap transition-colors flex items-center gap-1.5 ${activeEventTab === tab ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                          {tab}
                          <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${activeEventTab === tab ? 'bg-white/20' : 'bg-gray-200'}`}>{count}</span>
                        </button>
                      )
                    })}
                  </div>

                  <motion.div key={activeEventTab} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-3 max-h-60 overflow-y-auto pr-1">
                    {(() => {
                      ...
                      if (filteredEvents.length === 0) {
                        return <div className="text-center py-6 bg-gray-50 rounded-lg border border-dashed border-gray-200"><p className="text-sm text-gray-500">No {activeEventTab.toLowerCase()} scheduled.</p></div>;
                      }
                      return filteredEvents.map(event => {
                        ...
                        if (event.source === 'service_delivery') { Icon = FiBriefcase; typeColor = 'text-blue-600 bg-blue-50 border-blue-100'; badgeLabel = 'Delivery'; }
                        else if (event.source === 'service_expiry') { Icon = FiAlertCircle; typeColor = 'text-rose-600 bg-rose-50 border-rose-100'; badgeLabel = 'Expiry'; }
                        else if (event.source === 'task' || event.source === 'calendar_event') { Icon = FiCheckSquare; typeColor = 'text-emerald-600 bg-emerald-50 border-emerald-100'; badgeLabel = 'Task'; }
                        return (
                          <div key={event.id} className={`flex items-start gap-3 p-3 rounded-lg border ${typeColor} shadow-sm transition-all hover:shadow-md group`}>
                            <div className={`mt-0.5 p-1.5 rounded-md bg-white shadow-sm shrink-0`}><Icon className="h-4 w-4" /></div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center justify-between mt-1.5">
                                <p className="text-xs font-medium flex items-center gap-1">
                                  ...
                                  <span className={`${isToday ? 'text-rose-600 font-bold' : isTomorrow ? 'text-amber-600 font-bold' : 'text-gray-600'}`}>{dayLabel}</span>
                                </p>
                                <span className="text-[9px] uppercase tracking-wider font-bold opacity-70">{badgeLabel}</span>
                              </div>
                            </div>
                            <div className="shrink-0 flex flex-col items-center justify-center self-stretch ml-1">
                              <button onClick={() => handleViewService(event)} className="p-2 bg-white/60 hover:bg-white rounded-full shadow-sm text-gray-500 hover:text-indigo-600 transition-all border border-black/5 opacity-80 hover:opacity-100" title="Go to Service Tracking">
                    ...
                    {Object.entries(groupedRecentActivities).map(([date, entries]) => (
                      <div key={date} className="relative">
                        <div className="flex items-center gap-3 mb-4 sticky top-0 bg-white/90 backdrop-blur-sm py-2 z-20 -mx-2 px-2">
                          <div className="p-1.5 bg-gray-100 rounded-md"><FiCalendar className="h-3.5 w-3.5 text-gray-600" /></div>
                          <h4 className="text-xs font-bold text-gray-800 uppercase tracking-wider">{formatDateUI(date)}</h4>
                          <div className="h-px bg-gray-200 flex-1 ml-2"></div>
                        </div>
                        {/* 2‑column grid for activity cards */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {entries.map((entry, index) => (
                            <div key={entry.id} className="group relative flex gap-3 bg-white border border-gray-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-all hover:border-indigo-200">
                              <div className="w-9 h-9 bg-indigo-50 border border-indigo-100 rounded-full flex items-center justify-center shrink-0 z-10 transition-transform group-hover:scale-110">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                  <div className="flex flex-wrap items-center gap-1">
                                    ...
                                    {entry.workSource === 'online' && (
                                      <span className="flex items-center gap-0.5 px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded text-[10px] font-bold uppercase tracking-wider border border-blue-100">
                                        <FiGlobe className="h-2.5 w-2.5" /> Online
                                      </span>
                                    )}
                                    {entry.is_edited && (
                                      <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded text-[10px] font-bold uppercase tracking-wider border border-gray-200">Edited</span>
                                    )}
                                  </div>
                                  <span className="text-[10px] font-semibold text-gray-500 bg-gray-50 px-2 py-0.5 rounded-md shrink-0 border border-gray-100">
                                    {formatTime(entry.created_at)}
                                  </span>
                                </div>
                                <p className="text-xs font-medium text-gray-600 mb-2 truncate">{getCategoryName(entry.category)} service</p>
                                <div className="flex justify-between items-center pt-2 border-t border-gray-50">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {entry.tokenId && (
                                      <span className="text-[10px] text-gray-500 font-mono font-medium pl-1.5 border-l border-gray-200">
                                        {shortenTokenId(entry.tokenId)}
                                      </span>
                                    )}
                                  </div>
                                  {(entry.tokenId || entry.tracking_id) && (
                                    <button onClick={() => handleViewDetails(entry.tokenId, entry.tracking_id)} className="px-2.5 py-1 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider shadow-sm transition-all shrink-0">
                                      <FiBarChart2 className="h-3 w-3" /> Details
                                    </button>
                                  )}
                                </div>
                              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="mb-4">
              ...
              <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} rows="3" className="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., customer requested, duplicate entry..." />
            </div>
