...
import { 
  FiUsers, FiClock, FiCheckCircle, FiPlayCircle, FiPlus, 
  FiSearch, FiAlertCircle, FiRefreshCw, FiCalendar, FiBarChart2,
  FiTrendingUp, FiUser, FiAward, FiXCircle, FiCheckSquare
} from 'react-icons/fi';
...
const StaffDashboard = () => {
  ...
  const fetchPerformance = useCallback(async () => {
    try {
      // Get from /staff-performance/dashboard (adjust path if needed)
      const res = await api.get('/staff-performance/dashboard');
      if (res.data.success) {
        const { summary, ratings } = res.data.data;
        setPerformance({
          completionRate: summary.collection_rate || 0,
          avgServiceTime: 'N/A', // we'll compute separately if needed
        ...
        // Optionally compute average service time from daily_performance or recent_services
        // This is a fallback – you can also query the backend for it
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={refreshTokens} className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg" title="Refresh">
      ...
      <div className="p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
            {[
              { label: 'Total Tokens', value: statusCounts.total, icon: FiUsers, color: 'text-gray-600' },
              { label: 'Pending', value: statusCounts.pending, icon: FiClock, color: 'text-amber-500' },
              { label: 'In Progress', value: statusCounts.inProgress, icon: FiPlayCircle, color: 'text-blue-500' },
              { label: 'Completed', value: statusCounts.completed, icon: FiCheckCircle, color: 'text-green-500' },
              { label: 'Campaign', value: statusCounts.campaign, icon: FiAward, color: 'text-purple-500' },
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2 space-y-6">
              ...
              {/* Online Bookings Sections (unchanged layout) */}
                        ...
                        <div key={date}>
                          ...
                          <div className="space-y-3">
                            {dateTokens.map(token => {
                              ...
                              const isUnassigned = !tokenStaff || tokenStaff === 'null' || tokenStaff === '';
                              return (
                                <div key={token.tokenId} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-gray-300">
                                  <div className="flex items-center gap-4 flex-1">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => navigate('/staff/token/create')} className="p-3 border border-gray-300 rounded-lg hover:border-gray-400 text-left">
                    <FiPlus className="h-5 w-5 text-gray-600 mb-2" />
                    <span className="text-sm font-medium text-gray-900">New Token</span>
                  </button>
                  <button onClick={() => navigate('/staff/reports')} className="p-3 border border-gray-300 rounded-lg hover:border-gray-400 text-left">
                    <FiBarChart2 className="h-5 w-5 text-gray-600 mb-2" />
                    <span className="text-sm font-medium text-gray-900">Reports</span>
                  </button>
                  <button onClick={() => navigate('/staff/customers')} className="p-3 border border-gray-300 rounded-lg hover:border-gray-400 text-left">
                    <FiUsers className="h-5 w-5 text-gray-600 mb-2" />
                    <span className="text-sm font-medium text-gray-900">Customers</span>
                  </button>
                  <button onClick={refreshTokens} className="p-3 border border-gray-300 rounded-lg hover:border-gray-400 text-left">
