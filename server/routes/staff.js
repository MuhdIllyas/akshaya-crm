...
router.post("/add", authMiddleware(["admin", "superadmin"]), async (req, res) => {
  ...
  try {
    ...
    if (!username || !name || !role || !email || !password) {
      console.log(`Missing required fields: ${JSON.stringify({ username, name, role, email, password })}`);
    ...
    // Insert staff schedule if provided
    ...
    // ========== ACTIVITY LOGGING ==========
    // Log staff added activity
    await logActivity({
      centre_id: centreId,
      related_type: 'staff',
      related_id: newStaff.id,
      action: 'Staff Added',
      description: `New staff member ${name} (${role}) added to ${role === 'admin' ? 'centre as admin' : 'team'}`,
...
// Update the GET /api/staff/all route in staff.js
...
router.delete("/:id", authMiddleware(["admin", "superadmin"]), async (req, res) => {
  ...
  try {
    ...
    await logActivity({
      ...
      description: `Staff member ${staff.name} (${staff.username}) removed from system`,
      performed_by: req.user.id,
      performed_by_role: req.user.role
    });
    // ======================================
...
router.get("/schedule/:id", authMiddleware(["admin", "superadmin"]), async (req, res) => {
  ...
  try {
    ...
    if (role === "superadmin") {
      return res.json({}); // No schedule for superadmins
    }
    ...
    if (result.rows.length === 0) {
      return res.json({}); // No schedule found
    }
...
router.put("/:id", authMiddleware(["admin", "superadmin"]), async (req, res) => {
  ...
  try {
    ...
    // Update or insert staff schedule
    if (start_time && end_time && effective_from && role !== "superadmin") {
      const [startHour, startMinute] = start_time.split(':').map(Number);
      const [endHour, endMinute] = end_time.split(':').map(Number);
      let standardHours = endHour - startHour + (endMinute - startMinute) / 60;
